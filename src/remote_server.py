...
async def get_public_ip() -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            response = await client.get("http://169.254.169.254/latest/meta-data/public-ipv4")
...
async def mcp_message_handler(request: Request) -> JSONResponse:
    ...
    if not hasattr(request.app.state, 'mcp_sessions') or integration_id not in request.app.state.mcp_sessions:
        ...
        request.app.state.mcp_sessions[integration_id] = {'server': mcp_server, 'initialized': False}
    ...
    try:
        ...
        logger.info(f"Received MCP request for {integration_id[:8]}: method={body.get('method', 'unknown')}, body={json.dumps(body)}")
        ...
        if body.get('method') == 'initialize':
            ...
            try:
                ...
                response = {
                    ...
                    "id": body.get('id'),
                    "result": {
                        "content": [{"type": "text", "text": content.text} for content in text_contents]
                    }
                }
        ...
        if hasattr(request.app.state, 'sse_queues') and integration_id in request.app.state.sse_queues:
...
async def sse_endpoint(request: Request):
    ...
    async def event_generator():
        ...
        try:
            ...
            yield {
                "event": "endpoint",
                "data": "/mcp/messages/" + integration_id
            }
            ...
            while True:
                try:
                    ...
                    yield {
                        "event": "ping",
                        "data": json.dumps({"timestamp": time.time(), "counter": ping_counter})
                    }
            ...
            if hasattr(request.app.state, 'mcp_sessions') and integration_id in request.app.state.mcp_sessions:
...
app = Starlette(
    ...
    routes=[
        ...
        Route("/mcp/messages/{integration_id}", mcp_message_handler, methods=["POST"]),  # POST endpoint for MCP messages
